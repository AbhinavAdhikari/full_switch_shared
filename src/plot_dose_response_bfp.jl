...
PyPlot.clf()
PyPlot.figure(1)
# fill_between(log10.(gluconate_concentration), vec(UB_venus), vec(LB_venus), color="#5C995B", alpha=0.60)
PyPlot.fill_between(log10.(gluconate_concentration), vec(UB_bfp), vec(LB_bfp), color="#5C995B", alpha=0.40)
...
# PyPlot.errorbar(log10.(gluconate_concentration_exp), venus_mean_exp,yerr=yerr_array_venus,fmt="o",mfc="#5C995B",mec="#252525",color="#252525", lw=1.5)
PyPlot.errorbar(log10.(gluconate_concentration_exp), bfp_mean_exp,yerr=yerr_array_bfp,fmt="o",mfc="#252525",mec="#252525",color="#252525", lw=1.5)

# labels - 
PyPlot.plot(figsize=(4,4))
PyPlot.xlabel("log [Glucose](mM)", fontsize=16)
PyPlot.ylabel("[BFP] (μM)", fontsize=16)
# PyPlot.axis([-0.5,16.5,-50,1600])
PyPlot.xticks(fontsize=14)
PyPlot.yticks(fontsize=14)
PyPlot.tight_layout()
PyPlot.savefig("./plots/dose_response_plot_bfp_2.pdf")