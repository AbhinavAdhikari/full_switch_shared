...
function main(path_to_simulation_dir::String, path_to_plot_file::String, concentration)
    ...
    PyPlot.fill_between(t_intervals, vec(UB), vec(LB), color="#5c995b", alpha=0.45)
    # Plot mean -
    ...
    PyPlot.ylabel("[BFP] (μM)", fontsize=20)
    ...
    PyPlot.tight_layout()
    PyPlot.savefig("$(path_to_plot_file)/prot-bfp_$(concentration)mM_2.pdf")
end
