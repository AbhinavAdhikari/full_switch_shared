...
function main(path_to_ensemble_file::String)
    ...
    # get TX_bfp time constant mods -
    tc_mods_tx_ensemble = PA[23,:]
    number_of_genes = 1
    number_of_samples = length(tc_mods_tx_ensemble)
    ...
    # what is the length -
    prot_length = prot_coding_length_array[4]
    ...
    # compute the time constants for TL bfp -
    tc_mods_tl_ensemble = PA[28,:]
    number_of_prots = 1
    number_of_samples = length(tc_mods_tl_ensemble)
    ...
    # what is the length -
    prot_length = prot_coding_length_array[5]
    ...
    # compute half lifes for mRNA_bfp -
    deg_mod_mRNA = PA[33,:]
    number_of_mRNA = 1
    (number_of_samples) = length(deg_mod_mRNA)
    ...
    # return -
    return (S70_halflife,n_hill,K_hill,tau_tx_ensemble_gntr,tau_tx_ensemble_venus,tau_tl_ensemble_gntr, tau_tl_ensemble_venus, KL_array, half_life_TL_array, dE_array, mRNA_half_life_array_gntr, mRNA_half_life_array_venus, prot_half_life_array,n_array,K_array)
end

# setup -
path_to_ensemble_file = "simulated/poets_ensemble/PC_T6.dat"

# execute -
(S70_halflife,n_hill,K_hill,tau_tx_ensemble_gntr,tau_tx_ensemble_venus, tau_tl_ensemble_gntr,tau_tl_ensemble_venus, KL_array, half_life_TL_array, dE_array, mRNA_half_life_array_gntr, mRNA_half_life_array_venus, prot_half_life_array,n_array,K_array) = main(path_to_ensemble_file)

# compute some stats -
(μ_tx_venus,σ_tx_venus) = compute_some_stats(transpose(tau_tx_ensemble_gntr); dim_index = 1)
(μ_tx_bfp,σ_tx_bfp) = compute_some_stats(transpose(tau_tx_ensemble_venus); dim_index = 1)

(μ_tl_venus,σ_tl_venus) = compute_some_stats(transpose(tau_tl_ensemble_gntr); dim_index = 1)
(μ_tl_bfp,σ_tl_bfp) = compute_some_stats(transpose(tau_tl_ensemble_venus); dim_index = 1)
...
(μ_mRNA_half_life_venus,σ_mRNA_half_life_venus) = compute_some_stats(mRNA_half_life_array_gntr; dim_index=1)
(μ_mRNA_half_life_bfp,σ_mRNA_half_life_bfp) = compute_some_stats(mRNA_half_life_array_venus; dim_index=1)
